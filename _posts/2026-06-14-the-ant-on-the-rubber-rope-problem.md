...
This puzzle originates from none other than [Martin Gardner](https://en.wikipedia.org/wiki/Martin_Gardner). Here is the original problem statement and original diagrams from [Aha! Gotcha: Paradoxes to Puzzle and Delight](https://archive.org/details/ahagotchaparadox00gard/page/144/mode/2up) published in 1982.

> A worm is at one end of a rubber rope. The rope is 1 kilometer long. The worm crawls along the rope at a steady pace of 1 centimeter per second. After the first second, the rope stretches like a rubber band to 2 kilometers. After the next second, it stretches to 3 kilometers, and so on. Will the worm ever reach the end of its rope? Your intuition tells you that the worm will never reach the end. But does it! How long does it take?
...
The first fact describes the behavior of the _uniformly expanding rubber rope_. As the rope is pulled rightward, its rightmost edge is moving at velocity $u$ and its leftmost edge at velocity $0$. So intuitively all of the points in-between must linearly and continuously fill in that gap, i.e. the velocity of a point on the rope is proportional to its _initial_ displacement. In the diagram, this is represented by the growing length of the blue arrows.

The second fact is a consequence of the fact that the end of the rope is being pulled with constant velocity. Thus, every point on the rope is moving at a constant velocity. This is represented in the diagram by the fact that the length of the blue arrows associated with a segment do not change.

The third fact should be obvious. Suppose an ant is traveling at $1 \ \text{cm/s}$ and the end of the rope is expanding at $0.5 \ \text{cm/s}$. Then obviously the ant will eventually reach the end of the rope. Analogously, we can segment the rope into small enough chunks such that the ant is moving faster than the first segment. Thus, the ant will eventually overtake that segment. In the diagram, the ant's red arrow is larger than the first blue arrow, and therefore it can eventually reach the end of that segment.
...
The fifth fact is a consequence of the fourth. The problem says that the ant moves at a velocity $\alpha$ _relative_ to the rope. So its absolute velocity is $\alpha$ plus the velocity of the segment of the rope the ant is currently on.

Now let's put everything together. Segment the rope into small enough chunks such that the first segment is moving slower than the ant's velocity. Then obviously the ant will eventually overtake that segment. Once the ant gets to the end of that segment, its absolute velocity increases by the speed of that segment. Now, the ant is moving faster than the second segment, and thus will eventually overtake it. We can continue this reasoning to conclude that the ant will eventually get to the end of the rope.
...
<div class="overflow-container">
<div class="overflow-content">
<embed src="/blog-assets/the-ant-on-the-rubber-rope-problem/rope_segments.png" alt="Rope Segments" width="900px" />
</div>
</div>
</center>

Since the rope is expanding uniformly, the velocity of the right edge of each segment is constant over time and proportional to its fractional displacement. I will prove this more rigorously when solving for the exact solution. The velocity of the right edge of the $n\text{th}$ segment is given by
...
Now, we will use induction on $N$ (the number of rope segments) to show that the ant will eventually reach the end of the $N\text{th}$ segment. In particular, we will show there exists some time at which the ant has overtaken each segment in turn. And since at each segment the ant's speed exceeds that segment's velocity, it must eventually overtake it.
...
**Induction Step**: Suppose the ant eventually reaches the end of the $n\text{th}$ segment.

The right edge of the $n\text{th}$ rope segment is traveling at velocity $v_n$, so the absolute speed of the ant is at least $\alpha + v_n$. Therefore
...
Here, I provide a version of the argument originally presented by Martin Gardner. Like the induction proof, we are just showing that the ant will eventually reach the end of the rope.

Consider a discrete version of this problem where every $\Delta t$ seconds, the rope first expands by $u \Delta t$ and then the ant moves by $\alpha \Delta t$. Compared to the continuous problem, we are disadvantaging the ant by giving the rope more time to expand. So if the ant can reach the end of the rope in this modification, it can also do so in the original formulation.

### The Solution to the Original Problem Statement

This is a modified version of Martin Gardner's solution using the parameters $L_0 = 1 \ \text{km}$, $u = 1 \ \frac{\text{km}}{\text{s}}$, $x_0 = 0 \ \text{km}$, and $\alpha = 1 \ \frac{\text{cm}}{\text{s}} = 1/100000 \ \frac{\text{km}}{\text{s}}$.
...
- $t = 3$ - the rope is length $4 \ \text{km}$ and the ant has traversed $(1/200000 + 1/300000 + 1/400000)\text{th}$ of the rope.
- And so on

Let $\theta(t)$ denote the fraction of the rope that the ant has traversed. Based on the above reasoning, it is equal to

$$
\theta(t) = \frac{1}{100000} \sum_{k = 2}^{t+1} \frac{1}{k}
$$

The sum $\sum_{k = 2}^{t+1} \frac{1}{k}$ is called the [Harmonic Series](https://en.wikipedia.org/wiki/Harmonic_series_(mathematics)) and famously diverges [[source](https://proofwiki.org/wiki/Harmonic_Series_is_Divergent)], which means
...
Now for the generalized version of the above argument. First, let $t = n \Delta t$ where $\Delta t$ is the time step size and $n$ is the number of time steps. In each time step, the rope expands by $u \Delta t$ and afterwards the ant moves by $\alpha \Delta t$.


Let $L_n$ denote the rope's length after $n$ time steps. Per the problem statement, the right end of the rope moves at a constant velocity and thus
...
Notice that $\frac{1}{L_0 + (u \Delta t) k} \geq \frac{1}{L_0 k + (u \Delta t) k}$ for $k \geq 1$ (since $L_0 \leq L_0 k$ for $k \geq 1$, so the denominator on the left is no larger). Therefore

$$
\theta_n \geq \frac{x_0}{L_0} + \frac{\alpha \Delta t}{L_0 + (u \Delta t)}\sum_{k=1}^n \frac{1}{k}
$$

We can disregard the $\frac{x_0}{L_0}$ term since it is non-negative and only strengthens the bound. Also, the harmonic series is lower-bounded by $\ln(n + 1)$ [[source](https://math.stackexchange.com/questions/3883581/finding-upper-and-lower-bounds-of-the-harmonic-series)]. Therefore
...
Let $L(t)$ be the length of the rope at time $t$. Therefore $L(0) = L_0$. The right end of the rope moves at a constant velocity of $u$. Therefore
...
Since the rope expands uniformly, the fractional position of each point on the rope must remain constant. Therefore
...
The above formulation is a bit awkward to work with because the parameter $\ell_0$ refers to the original position on the rope at $t = 0$. But ideally, we want to be able to reference the rope's position relative to any point in time $t$.
...
Instead of picking an initial position and an arbitrary time, this formulation lets us pick an arbitrary position and an arbitrary time, which is much nicer to work with.
...
The distance traveled by the ant will have been
